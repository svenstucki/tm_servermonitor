...
class IXR_Client_Gbx {
	...
	// serverviewer.svenstucki.ch Modifications to get traffic usage
	public $traffic_in = 0;
	public $traffic_out = 0;
	...
	function InitWithIp($ip, $port) {
		...
		// open connection
		$this->socket = @fsockopen($ip, $port, $errno, $errstr, 4);
		if (!$this->socket) {
	...
	protected function sendRequest(IXR_Request $request) {
		...
		@stream_set_timeout($this->socket, 10);  // timeout 20 s (to write the request)
		// send request
		$this->reqhandle++;
		if ($this->protocol == 1) {
		...
		$this->traffic_out += $bytes_to_write;
	...
	function getResponse( $obj=false ) {
		// methodResponses can only have one param - return that
		if( $obj )
		    return (object) $this->message->params[0];
